    ...
    return new Promise(function (resolve, reject) {
        ref.once('value', async (snapshot) => {
            ...
            for (let obj of Object.values(snapshot.val().tokens)) {
                const tokenExists = snapshot.val() && snapshot.val().tokenHolders && snapshot.val().tokenHolders[obj.variableTokenSymbol]
                const start = tokenExists && snapshot.val().tokenHolders[obj.variableTokenSymbol].lastBlock
                    ? snapshot.val().tokenHolders[obj.variableTokenSymbol].lastBlock : 0
                const count = tokenExists && snapshot.val().tokenHolders[obj.variableTokenSymbol].count
                    ? snapshot.val().tokenHolders[obj.variableTokenSymbol].count : 0
                promises.push(getBorrowers(obj.variableDebtTokenAddress, start, count, snapshot.val()))
            }
