    ...
    return new Promise(function (resolve, reject) {
        ref.once('value', async (snapshot) => {
            ...
            for (let obj of Object.values(snapshot.val().tokens)) {
                const tokenExists = snapshot.val() && snapshot.val().tokenHolders && snapshot.val().tokenHolders[obj.stableTokenSymbol]
                const start = tokenExists && snapshot.val().tokenHolders[obj.stableTokenSymbol].lastBlock
                    ? snapshot.val().tokenHolders[obj.stableTokenSymbol].lastBlock : 0
                const count = tokenExists && snapshot.val().tokenHolders[obj.stableTokenSymbol].count
                    ? snapshot.val().tokenHolders[obj.stableTokenSymbol].count : 0
                promises.push(getBorrowers(obj.stableDebtTokenAddress, start, count, snapshot.val()))
            }
